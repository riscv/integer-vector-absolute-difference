...
=== Extension Overview
The Zvabd extension follows the Vector Arithmetic Instruction Formats.
...
| vwabdacc.vv        | <<insns-vwabdacc>>
| vwabdaccu.vv       | <<insns-vwabdaccu>>

|===

NOTE: Since the targeted use cases are mostly based on 8-bit and 16-bit integer, `vabd/vabdu/vwabdacc/vwabdaccu` are designed for only SEW=8 and SEW=16. Additionally, `vabs` is considered to support all the element sizes as it is quite common used and simple to implement.
...
[[insns-vwabdacc, Vector Signed Integer Absolute Difference And Accumulate]]
==== vwabdacc.vv
...
Mnemonic::
vwabdacc.vv _vd_, _vs2_, _vs1_, _vm_
...
[[insns-vwabdaccu, Vector Unsigned Integer Absolute Difference And Accumulate]]
==== vwabdaccu.vv
...
Mnemonic::
vwabdaccu.vv _vd_, _vs2_, _vs1_, _vm_
